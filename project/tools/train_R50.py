...
def main():
    ...
    For noisy student
    """
    # net_student, start_epoch = train_args.resume_train(net)
    # net_student.cuda()
    # net_student.train()
    ...
    train_set, val_set, unlabel_set = train_args.get_dataset()
    ...
    unlabel_loader = DataLoader(dataset=unlabel_set, batch_size=train_args.train_batch, num_workers=0, shuffle=True)
    ...
    # while True:
        ...
        for i, (inputs, labels) in enumerate(tqdm(unlabel_loader)):
            sys.stdout.flush()
            ...
            # outputs, cost = net_student(inputs)

            main_loss = criterion(outputs, pseudo_label)
            loss = main_loss + cost
    ...
    """
        ...
        avg_loss, acc, acc_cls, mean_iu, mean_iu_noback, fwavacc, f1 = validate(net, val_set, val_loader, criterion, optimizer, start_epoch + new_ep, new_ep)
        val_loss.append(avg_loss)
        mean_IU.append(mean_iu)
        mean_IU_no.append(mean_iu_noback)
        acc_list.append(acc)
        f1_list.append(f1)

        new_ep += 1

    plt.title('The validation loss each epoch')
    plt.plot(np.array(range(num_batch)), val_loss)
    ...
    plt.title('The accuracy each epoch')
    plt.plot(np.array(range(num_batch)), acc_list)
    plt.ylabel('Accuracy')
    plt.xlabel('Number of Epoch')
    plt.show()
    ...
    plt.title('The mean Intersection-over-Union (mIoU) each epoch')
    plt.plot(np.array(range(num_batch)), mean_IU)
    plt.ylabel('mIoU')
    plt.xlabel('Number of Epoch')
    plt.show()
    ...
    plt.title('The mIoU without background each epoch')
    plt.plot(np.array(range(num_batch)), mean_IU_no)
    plt.ylabel('mIoU')
    plt.xlabel('Number of Epoch')
    plt.show()

    print("The mIoU without background :")
    print(mean_IU_no)

    plt.title('The macro F1 score each epoch')
    plt.plot(np.array(range(num_batch)), f1_list)
    plt.ylabel('F1')
    plt.xlabel('Number of Epoch')
    plt.show()
...
def validate(net, val_set, val_loader, criterion, optimizer, epoch, new_ep):
    ...
    avg_loss, acc, acc_cls, mean_iu, mean_iu_noback, fwavacc, f1 = update_ckpt(net, optimizer, epoch, new_ep, val_loss,
                                                                     inputs_all, gts_all, predictions_all)

    # acc, acc_cls, mean_iu, mean_iu_noback, fwavacc, f1 = evaluate(predictions_all, gts_all, train_args.nb_classes)
    # val_visual = visual_ckpt(epoch, new_ep, inputs_all, gts_all, predictions_all)

    net.train()
    return avg_loss, acc, acc_cls, mean_iu, mean_iu_noback, fwavacc, f1
...
def update_ckpt(net, optimizer, epoch, new_ep, val_loss,
    ...
    writer.add_scalar('mean_iu_no_background', mean_iu_noback, epoch)
