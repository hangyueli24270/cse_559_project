...
class rx50_gcn_3head_4channel(nn.Module):
    def __init__(self, out_channels=7, pretrained=True,
        ...
        self.layer0 = torch.nn.Sequential(self.conv0, *list(self.layer0)[0:3])

        self.graph_layers1 = GCN_Layer(256, 64, bnorm=True, activation=nn.ReLU(True), dropout=dropout)

        self.graph_layers2 = GCN_Layer(64, out_channels, bnorm=False, activation=None)

        self.scg = SCG_block(in_ch=256,
